...
const DEFAULT_ADMIN_KEY = "pixel-admin-2026";
const DEFAULT_SESSION_SECRET = "pixel-workshop-dev-secret";

// 生产环境安全基线：关键密钥缺失或仍为默认值时直接拒绝启动。
function checkStartupConfig() {
  const missing = [];
  if (!process.env.CARD_ADMIN_KEY || process.env.CARD_ADMIN_KEY === DEFAULT_ADMIN_KEY) {
    missing.push("CARD_ADMIN_KEY 必须设置为强随机值（当前缺失或为默认值）");
  }
  if (!process.env.CARD_SESSION_SECRET || process.env.CARD_SESSION_SECRET === DEFAULT_SESSION_SECRET) {
    missing.push("CARD_SESSION_SECRET 必须设置为强随机值（当前缺失或为默认值）");
  }
  if (!process.env.VOLC_ACCESS_KEY_ID || !process.env.VOLC_SECRET_ACCESS_KEY) {
    missing.push("VOLC_ACCESS_KEY_ID / VOLC_SECRET_ACCESS_KEY 必须配置");
  }
  if (!missing.length) return;

  const message = `启动配置检查失败：\n- ${missing.join("\n- ")}`;
  if (IS_PRODUCTION) {
    console.error(message);
    process.exit(1);
  }
  console.warn(`[配置警告] ${message}\n本地开发可忽略；生产环境（NODE_ENV=production）将拒绝启动。`);
}
...
function hmac(key, data) {
  // 返回原始 Buffer 而非 hex，火山引擎签名链要求用二进制 HMAC 作为下一轮密钥。
...
async function callVolc(action, requestBody) {
  ...
  if (!accessKeyId || !secretAccessKey) {
    throw new Error("缺少鉴权密钥，请在 .dev.vars 中配置 VOLC_ACCESS_KEY_ID 和 VOLC_SECRET_ACCESS_KEY。");
  }
...
// Read full request body as text (regardless of content-type)
async function readRequestBodyAsText(request, maxBytes = MAX_DOWNLOAD_BODY_BYTES) {
  return readBodyLimited(request, maxBytes);
}

const DEFAULT_CLIENT_ORIGIN = process.env.CLIENT_ORIGIN || "http://127.0.0.1:8789";
const EXTRA_ALLOWED_ORIGINS = (process.env.ALLOWED_ORIGINS || "")
  .split(",")
  .map((item) => item.trim())
  .filter(Boolean);

// CORS 白名单：只反射可信来源，未知来源不返回 allow-origin，浏览器会直接拦截。
...
function getCorsOrigin(request) {
  ...
  if (origin && DEFAULT_ALLOWED_ORIGINS.has(origin)) return origin;
  return null;
}
...
function buildContentDisposition(filename) {
  ...
  // RFC 5987：同时输出 ASCII 兜底名与 UTF-8 编码名，保证中文文件名正确。
  const encoded = encodeURIComponent(filename).replace(/['()*]/g, (char) => `%${char.charCodeAt(0).toString(16).toUpperCase()}`);
  return `attachment; filename="${safeFilename}"; filename*=UTF-8''${encoded}`;
}

// 两段式下载的临时文件目录（供小程序 wx.downloadFile 使用），文件 10 分钟过期。
const DOWNLOAD_TMP_DIR = path.join(os.tmpdir(), "pixel-beads-downloads");
const DOWNLOAD_TMP_TTL_MS = 10 * 60 * 1000;

function ensureDownloadTmpDir() {
  if (!existsSync(DOWNLOAD_TMP_DIR)) mkdirSync(DOWNLOAD_TMP_DIR, { recursive: true });
}

function cleanStaleDownloadFiles() {
  ensureDownloadTmpDir();
  const now = Date.now();
  try {
    for (const entry of readdirSync(DOWNLOAD_TMP_DIR)) {
      const filePath = path.join(DOWNLOAD_TMP_DIR, entry);
      const stat = statSync(filePath);
      if (now - stat.mtimeMs > DOWNLOAD_TMP_TTL_MS) unlinkSync(filePath);
    }
  } catch {
    // 清理失败不影响主流程
  }
...
// Handle form POST download: accepts application/x-www-form-urlencoded with fields:
// - dataUrl: data:<mime>;base64,... (for images)
// - filename: suggested filename
// OR
// - text: raw text (for CSV), filename
// 下载接口：所有正式下载都必须经过这里，以便执行卡密校验与扣次。
async function handleDownload(request, response) {
  ...
  try {
    ...
    if (dataUrl) {
      ...
      console.log("Download filename:", filename, "content-disposition:", contentDisposition);
...
// 两段式下载第一步：校验卡密并扣次，把内容写入临时文件，返回可下载的 fileId。
// 主要供小程序 wx.downloadFile 使用（避免 wx.request 大体积 base64 超限）。
async function handleDownloadPrepare(request, response) {
  ...
  try {
    ...
    writeFileSync(path.join(DOWNLOAD_TMP_DIR, `${fileId}${ext}`), buffer);
...
// 两段式下载第二步：按 fileId 读取临时文件并流式返回（不再扣次）。
function handleDownloadFile(request, response) {
  ...
  const filePath = path.join(DOWNLOAD_TMP_DIR, fileId);
  const candidates = [filePath, `${filePath}.png`, `${filePath}.csv`].find((p) => existsSync(p) && statSync(p).isFile());
  if (!candidates) {
    response.writeHead(404, { "content-type": "application/json; charset=utf-8" });
    response.end(JSON.stringify({ error: "File not found or expired", message: "下载文件不存在或已过期，请重新导出。" }));
    return;
  }
  const ext = path.extname(candidates).toLowerCase();
  const mime = ext === ".csv" ? "text/csv; charset=utf-8" : "image/png";
  const contentDisposition = buildContentDisposition(`download${ext}`);
  response.writeHead(200, {
    "content-type": mime,
    "content-disposition": contentDisposition,
    "cache-control": "no-store",
  ...
  createReadStream(candidates).pipe(response);
}

// 授权状态接口：页面初始化时会调用它来恢复卡密状态。
...
// 卡密后台管理接口：支持列表、生成、重置，并返回最近操作日志。
async function handleCardAdmin(request, response) {
  try {
    await handleCardAdminAction(request, response);
  } catch (error) {
    sendJson(response, error?.statusCode || 500, {
...
async function handleCardAdminAction(request, response) {
...
// AI 优化接口：先校验卡密与图片绑定，再发起 AI 任务并扣减次数。
...
const server = http.createServer((request, response) => {
  ...
  if (request.url?.startsWith("/api/access-status")) {
    handleAccessStatus(request, response);
    return;
  }

  if (request.url?.startsWith("/api/redeem-card")) {
    handleRedeemCard(request, response);
    return;
  }

  if (request.url?.startsWith("/api/logout-access")) {
    handleLogoutAccess(request, response);
    return;
  }

  if (request.url?.startsWith("/api/card-admin/")) {
    handleCardAdmin(request, response);
    return;
  }

  if (request.url?.startsWith("/api/download-prepare")) {
    handleDownloadPrepare(request, response);
    return;
  }
...
server.listen(PORT, "0.0.0.0", () => {
  console.log(`拼豆项目服务已启动：http://127.0.0.1:${PORT}`);
});
